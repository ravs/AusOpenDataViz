function ParallelCoordinates(data,options) {
		...
		return d3.nest()
			...
			.rollup(function(leaves) {
				...
				options.columns.forEach(function(col){
					...
					r.round=leaves[0].round;
					r.name=leaves[0]["player_name"];
					r.name2=leaves[0]["player_name"];
				});
				return r;
			})
			.entries(data)
			...
			.sort(function(a,b){
				return d3.descending(a.values["MatchesPlayed"],b.values["MatchesPlayed"]);
			})
			.slice(0,30);
	}
	...
	function updateLabels(duration) {
		...
		var new_label=labels.enter()
					.append("g")
						.attr("class","label")
						.classed("round",function(d){
							return d.column=="round";
						});
		...
		new_label
			.filter(function(d){
				return d.column!="round" && d.column!=options.title_column;
			})
			.append("path");
